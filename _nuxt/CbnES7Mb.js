import"./zrhJ4IOG.js";const ls=""+new URL("cover.D7EDmnD9.webp",import.meta.url).href,cs=""+new URL("cover.DrB9i5VT.webp",import.meta.url).href,us=`---\r
date: 2026-05-16\r
...
`&&v!=="	"&&v!==" ")break}let d=t.substring(h);const g=d.indexOf(`
`);g===-1?u="-":t===d||g!==d.length-1?(u="+",i&&i()):u="",d&&(t=t.slice(0,-d.length),d[d.length-1]===`
`&&(d=d.slice(0,-1)),d=d.replace(bt,`$&${l}`));let p=!1,m,y=-1;for(m=0;m<t.length;++m){const v=t[m];if(v===" ")p=!0;else if(v===`
...
$&`).replace(/(?:^|\n)([\t ].*)(?:([\n\t ]*)\n(?![\n\t ]))?/g,"$1$2").replace(/\n+/g,`$&${l}`);let S=!1;const N=Ze(s,!0);o!=="folded"&&e!==A.BLOCK_FOLDED&&(N.onOverflow=()=>{S=!0});const w=Xe(`${b}${v}${d}`,l,gt,N);if(!S)return`>${T}
...
`&&(u+=`
`);break;default:u+=`
`}const g=s+r.length+e.source.length;return{value:u,type:i,comment:r.comment,range:[s,g,g]}}function Hs({offset:n,props:e},t,s){if(e[0].type!=="block-scalar-header")return s(e[0],"IMPOSSIBLE","Block scalar header not found"),null;const{source:r}=e[0],i=r[0];let o=0,a="",c=-1;for(let h=1;h<r.length;++h){const d=r[h];if(!a&&(d==="-"||d==="+"))a=d;else{const g=Number(d);!o&&g?o=g:c===-1&&(c=n+h)}}c!==-1&&s(c,"UNEXPECTED_TOKEN",`Block scalar header includes extra characters: ${r}`);let l=!1,f="",u=r.length;for(let h=1;h<e.length;++h){const d=e[h];switch(d.type){case"space":l=!0;case"newline":u+=d.source.length;break;case"comment":t&&!l&&s(d,"MISSING_CHAR","Comments must be separated from other tokens by white space characters"),u+=d.source.length,f=d.source.substring(1);break;case"error":s(d,"UNEXPECTED_TOKEN",d.message),u+=d.source.length;break;default:{const g=`Unexpected token in block scalar header: ${d.type}`;s(d,"UNEXPECTED_TOKEN",g);const p=d.source;p&&typeof p=="string"&&(u+=p.length)}}}return{mode:i,indent:o,chomp:a,comment:f,length:u}}function Xs(n){const e=n.split(/\n( *)/),t=e[0],s=t.match(/^( *)/),i=[s?.[1]?[s[1],t.slice(s[1].length)]:["",t]];for(let o=1;o<e.length;o+=2)i.push([e[o],e[o+1]]);return i}function Zs(n,e,t){const{offset:s,type:r,source:i,end:o}=n;let a,c;const l=(h,d,g)=>t(s+h,d,g);switch(r){case"scalar":a=A.PLAIN,c=er(i,l);break;case"single-quoted-scalar":a=A.QUOTE_SINGLE,c=tr(i,l);break;case"double-quoted-scalar":a=A.QUOTE_DOUBLE,c=nr(i,l);break;default:return t(n,"UNEXPECTED_TOKEN",`Expected a flow scalar value, but found: ${r}`),{value:"",type:null,comment:"",range:[s,s+i.length,s+i.length]}}const f=s+i.length,u=Pe(o,f,e,t);return{value:c,type:a,comment:u.comment,range:[s,f,u.offset]}}function er(n,e){let t="";switch(n[0]){case"	":t="a tab character";break;case",":t="flow indicator character ,";break;case"%":t="directive indicator character %";break;case"|":case">":{t=`block scalar indicator ${n[0]}`;break}case"@":case"`":{t=`reserved character ${n[0]}`;break}}return t&&e(0,"BAD_SCALAR_START",`Plain value cannot start with ${t}`),Vn(n)}function tr(n,e){return(n[n.length-1]!=="'"||n.length===1)&&e(n.length,"MISSING_CHAR","Missing closing 'quote"),Vn(n.slice(1,-1)).replace(/''/g,"'")}function Vn(n){let e,t;try{e=new RegExp(`(.*?)(?<![ 	])[ 	]*\r?
`,"sy"),t=new RegExp(`[ 	]*(.*?)(?:(?<![ 	])[ 	]*)?\r?
...
`:case"\r":case"	":return!0;default:return!1}}const sn=new Set("0123456789ABCDEFabcdef"),dr=new Set("0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz-#;/?:@&=+$_.!~*'()"),je=new Set(",[]{}"),mr=new Set(` ,[]{}
\r	`),dt=n=>!n||mr.has(n);class gr{constructor(){this.atEnd=!1,this.blockScalarIndent=-1,this.blockScalarKeep=!1,this.buffer="",this.flowKey=!1,this.flowLevel=0,this.indentNext=0,this.indentValue=0,this.lineEndPos=null,this.next=null,this.pos=0}*lex(e,t=!1){if(e){if(typeof e!="string")throw TypeError("source is not a string");this.buffer=this.buffer?this.buffer+e:e,this.lineEndPos=null}this.atEnd=!t;let s=this.next??"stream";for(;s&&(t||this.hasChars(1));)s=yield*this.parseNext(s)}atLineEnd(){let e=this.pos,t=this.buffer[e];for(;t===" "||t==="	";)t=this.buffer[++e];return!t||t==="#"||t===`
...
${l}`:l,r=r?`${r}
${f}`:f;let u=this.lexer.state.top;if(this.lexer.state.top=!0,this.lexer.blockTokens(f,i,!0),this.lexer.state.top=u,t.length===0)break;let h=i.at(-1);if(h?.type==="code")break;if(h?.type==="blockquote"){let d=h,g=d.raw+`
`+t.join(`
`),p=this.blockquote(g);i[i.length-1]=p,s=s.substring(0,s.length-d.raw.length)+p.raw,r=r.substring(0,r.length-d.text.length)+p.text;break}else if(h?.type==="list"){let d=h,g=d.raw+`
`+t.join(`
`),p=this.list(g);i[i.length-1]=p,s=s.substring(0,s.length-h.raw.length)+p.raw,r=r.substring(0,r.length-d.raw.length)+p.raw,t=g.substring(i.at(-1).raw.length).split(`
